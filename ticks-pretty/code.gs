function onOpen(e) {
  ...
  if (ScriptApp.getUserTriggers(DocumentApp.getActiveDocument())) {
    ScriptApp.getUserTriggers(DocumentApp.getActiveDocument()).forEach(function(
      trigger
    ) {
...
function ticks() {
  var regex = "`{1}(\\w|\\d|[^`+])+`{1}", // double '\' to escape character
    hasTicks = DocumentApp.getActiveDocument()
      .getBody()
      .findText(regex),
    startPosition,
    endPosition;
...
function retrievetripleTicks(regex) {
  var ticksExist = DocumentApp.getActiveDocument()
      .getBody()
      .findText(regex),
...
function tripleTicks() {
  ...
  for (var i = 0, x = pairs.length; i < x; i++) {
    range = rangeBuilder
      .addElementsBetween(pairs[i][0].getElement(), pairs[i][1].getElement())
      .build(); // build a range that is between the ``` pairs
    ...
    rangeElements.forEach(function(rangeElement) {
      ...
      element.setAttributes(style).replaceText(regex, "");
      if (hasComment) {
